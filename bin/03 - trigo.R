```{r}
pacman::p_load(tidyverse, tidyselect)
```


# Importación
```{r}
url_trigo <- "https://docs.google.com/spreadsheets/d/1QJuQ2Zm26ufVYklB7vak6gQej3Xtyvbtj1yPKDlczD0/edit#gid=407239570"
# browseURL(url)
trigo <- gsheet::gsheet2tbl(url_trigo)
...
```{r}
trigo %>%
  mutate_at(vars(contains('.')), funs(factor)) %>%
...
- reorganizar dataset: wide -> long

```{r}
trigo %>% 
  pivot_longer(contains("_"), 
               names_to = "org.var", values_to = "y") %>% 
  separate(org.var, c("org.", "var.")) %>% 
  mutate_at(vars(contains('.')), funs(factor)) %>%  
